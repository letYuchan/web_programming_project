...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchCards() {
        try {
            const response = await fetch('https://example.com/api/random-cards'); // 이름/SNS 제외 데이터 요청
            if (!response.ok) {
    ...
    async function fetchDetailedInfo(cardId) {
        try {
            const response = await fetch(`https://example.com/api/card-details?id=${cardId}`); // 상세 정보 요청
            if (!response.ok) {
